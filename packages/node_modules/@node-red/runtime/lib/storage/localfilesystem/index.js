    ...
    init: async function(_settings, runtime) {
        ...
        if (!settings.userDir) {
            if (checkForConfigFile(process.env.NODE_RED_HOME)) {
                ...
                settings.userDir = fspath.join(process.env.HOMEPATH,".nerco-ai");
            } else {
                settings.userDir = fspath.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH || process.env.NODE_RED_HOME,".nerco-ai");
            }
        }
