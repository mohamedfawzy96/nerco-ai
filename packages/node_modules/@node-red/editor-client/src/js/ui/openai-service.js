...
3. UPDATE EXISTING FLOWS
   ...
   - Generate the entire update flow in one go. Do not generate partial updates.
   - Include all properties coming from the current flow even if they are not being changed.
   - Ensure wires are correctly specified for all nodes
   - Ensure you are updating "outputs" and "inputs" properties of nodes correctly. "outputs" is number of output wires, "inputs" is number of input wires.
...
- For flow updates: update_flow(valid JSON string with updated flow and explanation)`
    ...
    function getFunctionDefinitions() {
        return [
            ...
            {
                name: "update_flow",
                description: "Updates an existing flow based on the user description. Generate the entire update flow in one go. Do not generate partial updates.",
                parameters: {
                    type: "object",
                    properties: {
                        flow: {
                            type: "string",
                            description: "The stringified JSON representation of the update Node-RED flow that can be imported directly. Must be a valid JSON string."
                        },
                        explanation: {
                            type: "string",
                            description: "A detailed explanation of the flow, how it works, and any special considerations"
                        ...
                        changes: {
                            type: "array",
                            description: "A list of changes applied to the flow",
                            items: {
                                type: "object",
                                properties: {
                                    nodeId: {type: "string"},
    ...
    function handleUpdateFlow(functionArgs) {
        try {
            return {
                ...
                isFlow: false,
                isUpdate: true,
                flow: JSON.parse(functionArgs.flow),
                changes: functionArgs.changes,
                description: functionArgs.explanation
            };
        } catch (e) {
            return {
